...
namespace TouchScript.Core
{
    ...
    internal sealed class GestureManagerInstance : MonoBehaviour, IGestureManager
    {
        ...
        /// <inheritdoc />
        public IGestureDelegate GlobalGestureDelegate
        {
            get;
            set;
        }
        ...
        private void sortPointersForActiveGestures(IList<Pointer> pointers)
        {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (!pointerToGestures.TryGetValue(pointer.Id, out gestures)) {
                    continue;
                }
        ...
        private void removePointers(IList<Pointer> pointers)
        {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (!pointerToGestures.TryGetValue(pointer.Id, out list)) {
                    continue;
                }
        ...
        private void resetGestures()
        {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (Equals(gesture, null)) {
                    continue;    // Reference comparison
                }
                ...
                if (gesture == null) {
                    continue;    // Unity "null" comparison
                }
        ...
        {
            foreach (var kv in hierarchyEndingWithCache) {
                gestureListPool.Release(kv.Value);
            }

            foreach (var kv in hierarchyBeginningWithCache) {
                gestureListPool.Release(kv.Value);
            }
        ...
        private List<Gesture> getHierarchyEndingWith(Transform target)
        {
            ...
            if (hierarchyEndingWithCache.TryGetValue(target, out list)) {
                return list;
            }
        ...
        {
            if (gesture.gameObject.activeInHierarchy == false) {
                return false;
            }
        ...
        private bool recognizeGestureIfNotPrevented(Gesture gesture)
        {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (!gestureIsActive(otherGesture)) {
                    continue;
                }
