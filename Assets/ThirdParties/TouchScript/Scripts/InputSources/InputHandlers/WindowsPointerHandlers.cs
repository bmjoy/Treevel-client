    ...
    public class Windows8PointerHandler : WindowsPointerHandler
    {
        ...
        public override bool CancelPointer(Pointer pointer, bool shouldReturn)
        {
            if (pointer.Equals(mousePointer)) {
                ...
                if (shouldReturn) {
                    mousePointer = internalReturnMousePointer(mousePointer);
                } else {
                    mousePointer = internalAddMousePointer(pointer.Position);    // can't totally cancel mouse pointer
                }
            ...
            if (pointer.Equals(penPointer)) {
                ...
                if (shouldReturn) {
                    penPointer = internalReturnPenPointer(penPointer);
                }
        ...
        {
            if (pointer is MousePointer) {
                mousePool.Release(pointer as MousePointer);
            } else if (pointer is PenPointer) {
                penPool.Release(pointer as PenPointer);
            } else {
    ...
    public abstract class WindowsPointerHandler : IInputSource, IDisposable
    {
        ...
        public virtual bool CancelPointer(Pointer pointer, bool shouldReturn)
        {
            ...
            if (internalTouchId > -1) {
                ...
                if (shouldReturn) {
                    winTouchToInternalId[internalTouchId] = internalReturnTouchPointer(touch);
                }
        ...
        {
            foreach (var i in winTouchToInternalId) {
        ...
        {
            if (penPointer != null) {
                throw new InvalidOperationException("One pen pointer is already registered! Trying to add another one.");
            }
        ...
        private float getTouchRotation(ref PointerData data)
        {
            ...
            if (reliable) {
                return data.Rotation / 180f * Mathf.PI;
            }
