    ...
    public class TouchHandler : IInputSource, IDisposable
    {
        ...
        public bool CancelPointer(Pointer pointer, bool shouldReturn)
        {
            ...
            if (fingerId > -1) {
                ...
                if (shouldReturn) {
                    systemToInternalId[fingerId] = new TouchState(internalReturnPointer(touch));
                } else {
                    systemToInternalId[fingerId] = new TouchState(touch, TouchPhase.Canceled);
                }
        ...
        public void Dispose()
        {
            foreach (var touchState in systemToInternalId) {
                if (touchState.Value.Phase != TouchPhase.Canceled) {
                    internalCancelPointer(touchState.Value.Pointer);
                }
            }
