...
namespace TouchScript.Debugging.GL
{
    public class GLDebug : MonoBehaviour
    {
        ...
        {
            return drawFigure(id, new List<Line>() {
                new Line(start, end)
            }, color ?? Color.white, duration, depthTest);
        }
        ...
        {
            return drawFigureScreenSpace(id, new List<Line>() {
                new Line(start, end)
        ...
        private static int drawFigure(int? id, List<Line> lines, Color color, float duration = 0, bool depthTest = false)
        {
            ...
            if (depthTest) {
                if (color == MULTIPLY) {
                    instance.figuresMultiplyDepthTest.Add(figureId, new Figure(figureId, lines, Color.white, duration));
                } else {
                    instance.figuresDepthTest.Add(figureId, new Figure(figureId, lines, color, duration));
                }
            } else {
                if (color == MULTIPLY) {
                    instance.figuresMultiplyNoDepthTest.Add(figureId, new Figure(figureId, lines, Color.white, duration));
                } else {
                    instance.figuresNoDepthTest.Add(figureId, new Figure(figureId, lines, color, duration));
                }
            }
        ...
        private static int drawFigureScreenSpace(int? id, List<Line> lines, Color color, float duration = 0)
        {
            ...
            if (color == MULTIPLY) {
                ...
                instance.figuresScreenSpace.Add(figureId, new Figure(figureId, lines, color, duration));
            }
